...
AI_PROMPTS = {
    ...
    "standup": (
        ...
        "Under Blockers, include ONLY tasks whose status is literally Blocker (or Pending) in the input. "
        "Do not put a task under Blockers just because its note contains a word like pending, waiting, or "
        "awaiting; a task that is merely awaiting review or a decision but is not tagged Blocker/Pending "
        "belongs under Today, not Blockers. For each real blocker, state what is blocked, why, and what "
        "is needed to unblock it. "
        "Every single task present in the input must appear exactly once in the output, under whichever "
        "section matches its status. Do not skip, drop, or silently omit any task, even if it looks "
        "similar to another one already covered. "
